TITLE_POINTS = {
    "World Cup winner": 100,
    "UEFA Champions League winner": 98,
    "European Champion Clubs' Cup winner": 98,
    "Copa Libertadores winner": 97,
    "FIFA Club World Cup winner": 96,
    "Intercontinental Cup Winner": 95,
    "Intercontinental Cup winner": 95,
    "European champion": 94,
    "FIFA Intercontinental Cup Winner": 93,
    "Copa América winner": 93,
    "Africa Cup winner": 88,
    "UEFA Supercup winner": 88,
    "Europa League winner": 87,
    "Uefa Cup winner": 87,
    "Europapokal der Pokalsieger Sieger": 86,
    "Copa Sudamericana winner": 86,
    "Asian Cup winner": 86,
    "AFC senior": 86,
    "CAF Champions League winner": 85,
    "AFC Champions League winner": 84,
    "CONCACAF Championship Winner": 84,
    "Inter-Cities Fairs Cup winner":83,
    "CONCACAF Champions Cup winner": 83,
    "CONCACAF Champions League winner": 83,
    "Gold Cup winner": 82,
    "Confederations Cup winner": 82,
    "Conference League winner": 80,
    "Copa CONMEBOL winner": 78,
    "CONMEBOL-UEFA Cup of Champions winner": 78,
    "Copa Mercosur Winner": 77,
    "Asian Cup Winners' Cup Winner": 77,
    "Supercopa Sudamericana winner": 76,
    "African Cup Winners' Cup Sieger": 76,
    "Winner UEFA Nations League": 76,
    "Copa Interamericana winner": 75,
    "Asian Super Cup Winner": 75,
    "CAF Super Cup Winner": 75,
    "Olympic medalist": 75,
    "Mitropa Cup winner": 74,
    "AFC Cup Winner": 74,
    "European Under-21 champion": 74,
    "CONCACAF League-Winner": 73,
    "CONCACAF Central American Cup Winner": 72,
    "Central American Cup winner": 72,
    "Asian Games Gold Medal": 72,
    "Leagues Cup Winner": 71,
    "Derby of the Americas Champion": 70,
    "Campeones Cup Winner": 70,
    "OFC Nations Cup winner": 70,
    "Copa Suruga Bank Winner": 69,
    "Royal League winner": 68,
    "AFC U23 Championship Winner": 68,
    "Afro-Asian Cup Winner": 66,
    "CONCACAF Nations League Winner": 66,
    "Under-20 World Cup champion": 65,
    "International Football Cup-Winner": 64,
    "OFC Champions League winner": 63,
    "AFC President's Cup Winner": 62,
    "AFC U20 Asian Cup Champion": 62,
    "GCC Champions League Winner": 61,
    "ASEAN Club Championship Winner": 60,
    # "UI Cup winner": 60,
    "CONCACAF-U20-Championship-Champion": 60,
    "Under-20 South American Championship winner": 60,
    "UNCAF Club Winner": 58,
    "Balkan Cup winner": 58,
    "AFF Championship winner": 58,
    "Under-20 Africa Cup winner": 58,
    "European Under-19 champion": 57,
    "Under-17 South American Championship winner": 56,
    "Under-17 Africa Cup winner": 55,
    "European Under-18 champion": 55,
    "Under-17 World Cup champion": 55,
    "European Under-17 champion": 54,
    "Under 16 European Champion": 52,
    "WAFF West Asian U23 Championship winner": 52,
    "AFC U16 Championship Winner": 50,
    "West Asian Championship winner": 50,
    "Central American and Caribbean Games Winner": 48,
    "Caribbean Cup winner": 46,
    "CAFA Nations Cup winner": 44,
    "East Asian Championship winner": 44,
    "Arab Cup winner": 42,
    "Pan American Games winner": 38,
    "Panamerican Champion": 35,
    "Winner South American Games": 30,
    "FIFA African-Asian-Pacific Cup Champion": 25,
    "UI Cup winner": 20
}

INDIVIDUAL_AWARD_POINTS = {
    "Ballon d'Or": 100,
    "FIFA World Player of the Year": 90,
    "The Best FIFA Men's Player": 90,
    "UEFA Best Player in Europe": 80,

    "World Cup Golden Ball": 70,

    "Top goal scorer": 20,
    "Best assist provider": 15,
}

NATIONAL_WORLD_TITLE = [
    "Arab Cup winner",
    "CONMEBOL-UEFA Cup of Champions winner",
    "Confederations Cup winner",
    "FIFA African-Asian-Pacific Cup Champion",
    "Olympic medalist",
    "Pan American Games winner",
    "Panamerican Champion",
    "Under-17 World Cup champion",
    "Under-20 World Cup champion",
    "Winner South American Games",
    "World Cup winner",
]

NATIONAL_CONTINENTAL_TITLE = [
    "AFC Challenge Cup Winner",
    "AFC U16 Championship Winner",
    "AFC U20 Asian Cup Champion",
    "AFC U23 Championship Winner",
    "AFF Championship winner",
    "AFF U16 Championship Winner",
    "AFF U23 Championship Winner",
    "AFF U19 Championship winner",
    "Africa Cup winner",
    "Arabian Gulf Cup winner",
    "Asian Cup winner",
    "Asian Games Gold Medal",
    "CAFA Nations Cup winner",
    "CONCACAF Championship Winner",
    "CONCACAF Nations League Winner",
    "CONCACAF-U20-Championship-Champion",
    "Caribbean Cup winner",
    "Central American and Caribbean Games Winner",
    "Copa América winner",
    "East Asian Championship winner",
    "European Under-17 champion",
    "European Under-18 champion",
    "European Under-19 champion",
    "European Under-21 champion",
    "European champion",
    "Gold Cup winner",
    "OFC Nations Cup winner",
    "OFC-U16/U17-Championship-Winner",
    "OFC U19/U20 Championship Winner",
    "South Asian Champion",
    "Southeast Asian Games Winner",
    "Under 16 European Champion",
    "Under-15 South American Championship winner",
    "Under-17 Africa Cup winner",
    "Under-17 South American Championship winner",
    "Under-20 Africa Cup winner",
    "Under-20 South American Championship winner",
    "Under-23 Africa Cup winner",
    "WAFF West Asian U23 Championship winner",
    "West Asian Championship winner",
    "Winner African Nations Championship",
    "Winner UEFA Nations League",
]

CLUB_INTERNATIONAL_TITLE = [
    "AFC Champions League winner",
    "AFC Cup Winner",
    "AFC President's Cup Winner",
    "ASEAN Club Championship Winner",
    "African Cup Winners' Cup Sieger",
    "African Football League Winner",
    "Afro-Asian Cup Winner",
    "Asian Club Championship winner",
    "Asian Cup Winners' Cup Winner",
    "Asian Super Cup Winner",
    "Balkan Cup winner",
    "CAF Champions League winner",
    "CAF Confederation Cup winner",
    "CAF Super Cup Winner",
    "CONCACAF Central American Cup Winner",
    "CONCACAF Champions Cup winner",
    "CONCACAF Champions League winner",
    "CONCACAF League-Winner",
    "Campeones Cup Winner",
    "Central American Cup winner",
    "Conference League winner",
    "Copa CONMEBOL winner",
    "Copa Interamericana winner",
    "Copa Libertadores winner",
    "Copa Sudamericana winner",
    "Copa Suruga Bank Winner",
    "Derby of the Americas Champion",
    "Europa League winner",
    "Europapokal der Pokalsieger Sieger",
    "European Champion Clubs' Cup winner",
    "FIFA Club World Cup winner",
    "FIFA Intercontinental Cup Winner",
    "GCC Champions League Winner",
    "Inter-Cities Fairs Cup winner",
    "Intercontinental Cup Winner",
    "Intercontinental Cup winner",
    "International Football Cup-Winner",
    "Intertoto-Cup Winner",
    "Leagues Cup Winner",
    "Mitropa Cup winner",
    "OFC Champions League winner",
    "Recopa Sudamericana winner",
    "Royal League winner",
    "Supercopa Sudamericana winner",
    "Thai Queen’s Cup Winner",
    "UEFA Champions League winner",
    "UI Cup winner",
    "UNCAF Club Winner",
    "Uefa Cup winner",
    "Uefa Supercup winner",
    "Winner Copa Mercosur",
]

CLUB_NATIONAL_TITLE = [
    "Albanian Cup winner",
    "Albanian champion",
    "Albanian Super Cup winner",
    "Algerian champion",
    "Algerian cup winner",
    "Algerian Super Cup Winner",
    "Andorran champion",
    "Andorran cup winner",
    "Andorran Supercup Winner",
    "Angolan Champion",
    "Angolan Cup Winner",
    "Argentine League Cup Winner",
    "Argentinian Cup Winner",
    "Argentinian champion",
    "Armenian Super Cup winner",
    "Armenian champion",
    "Armenian cup winner",
    "Australian champion",
    "Australian cup winner",
    "Austrian Super Cup winner",
    "Austrian champion",
    "Austrian cup winner",
    "Azerbaijani champion",
    "Azerbaijani cup winner",
    "Bahraini Champion",
    "Bahraini FA Cup Winner",
    "Bahraini King's Cup Winner",
    "Bahraini Supercup Winner",
    "Bengali Federation Cup Winner",
    "Belarusian Super Cup winner",
    "Belarusian champion",
    "Belarusian cup winner",
    "Belgian League Cup Winner",
    "Belgian Super Cup Winner",
    "Belgian champion",
    "Belgian cup winner",
    "Bengali Champion",
    "Bolivian champion",
    "Bosnian-Herzegovinian Super Cup winner",
    "Bosnian-Herzegovinian champion",
    "Bosnian-Herzegovinian cup winner",
    "Brazilian champion",
    "Brazilian cup winner",
    "Bulgarian Super Cup winner",
    "Bulgarian champion",
    "Bulgarian cup winner",
    "Cambodian League Cup winner",
    "Cambodian Supercup Winner",
    "Campeón Copa Ecuador",
    "Campeón Liga Profesional",
    "Campeón Supercopa Argentina",
    "Campeón Supercopa Ecuador",
    "Campeón Supercopa Internacional",
    "Campeón Trofeo de Campeones",
    "Canadian Champion",
    "Canadian Cup Winner",
    "Champion of Primeira Liga do Brasil",
    "Chilean Cup Winner",
    "Chilean Super Cup Winner",
    "Chilean champion",
    "Chinese Super Cup winner",
    "Chinese champion",
    "Chinese cup winner",
    "Colombian Champion",
    "Colombian Cup winner",
    "Colombian Super Cup winner",
    "Copa Eva Duarte winner",
    "Costa Rican Recopa Winner",
    "Costa Rican Super Cup winner",
    "Costa Rican champion Invierno",
    "Costa Rican champion Verano",
    "Costa Rican cup winner",
    "CPL Fall Season Champion",
    "Croatian Super Cup Winner",
    "Croatian champion",
    "Croatian cup winner",
    "Cypriot Super Cup Winner",
    "Cypriot champions",
    "Cypriot cup winner",
    "Czech champion",
    "Czech cup winner",
    "Czechoslovak champion",
    "Czechoslovak cup winner",
    "DDR champion",
    "DDR cup winner",
    "Danish Cup Winner",
    "Danish champion",
    "Dominican Champion",
    "DR Kongolesischer Meister",
    "Durand Cup Winner",
    "Dutch Cup winner",
    "Dutch Super Cup winner",
    "Dutch champion",
    "Ecuadorian champion",
    "Egyptian League Cup Winner",
    "Egyptian Super Cup Winner",
    "Egyptian champion",
    "Egyptian cup winner",
    "English Champion",
    "English FA Cup winner",
    "English League Cup winner",
    "English Super Cup winner",
    "Estonian cup winner",
    "Estonian Super Cup winner",
    "Estonian champion",
    "Ethiopian Champion",
    "Faroese champion",
    "Faroese cup winner",
    "Finnish champion",
    "Finnish cup winner",
    "Finnish league cup winner",
    "French Super Cup winner",
    "French champion",
    "French cup winner",
    "French league cup winner",
    "French occupation zone champion",
    "Ghanaian Champion",
    "Georgian Supercup winner",
    "Georgian champion",
    "Georgian cup winner",
    "German Champion",
    "German League Cup winner",
    "German Super Cup winner",
    "German cup winner",
    "Gibraltar Champion",
    "Gibraltar Cup Winner",
    "Gibraltar Supercup Winner",
    "Greek Super Cup winner",
    "Greek champion",
    "Greek cup winner",
    "Guatemalan Champion",
    "Guatemalan Champion Apertura",
    "Guatemalan Champion Clausura",
    "Honduran Champion Apertura",
    "Honduran Champion Clausura",
    "Honduran cup winner",
    "Honduran Super Cup winner",
    "Hong Kong League Cup winner",
    "Hong Kong Senior Challenge Shield Winner",
    "Hong Kong champion",
    "Hong Kong cup winner",
    "Hongkong Sapling Cup Winner",
    "Hungarian champion",
    "Hungarian cup winner",
    "Hungarian league cup winner",
    "Icelandic champion",
    "Icelandic cup winner",
    "Icelandic league cup winner",
    "Indian Champion",
    "Indian Federation Cup Winner",
    "Indian Super Cup Winner",
    "Indian Super League Champion",
    "Indonesian Champion",
    "Indonesian Community Shield Winner",
    "Indonesian Cup Winner",
    "Iraqi Champion",
    "Iraqi Cup Winner",
    "Iraqi Supercup Winner",
    "Iranian Supercup-Champion",
    "Iranian champion",
    "Iranian cup winner",
    "Irish champion",
    "Irish cup winner",
    "Irish league cup winner",
    "Israel Super Cup Winner",
    "Israeli champion",
    "Israeli cup winner",
    "Italian Super Cup winner",
    "Italian champion",
    "Italian cup winner",
    "Jamaican Champion",
    "Japan Soccer League Champion",
    "Japanese Super Cup winner",
    "Japanese champion",
    "Japanese cup winner",
    "Japanese league cup winner",
    "Jordanian Champion",
    "Jordanian Cup Winner",
    "Jordanian Supercup Winner",
    "Jordanischer FA Shield-Sieger",
    "Kazakh champion",
    "Kazakh cup winner",
    "Kazakh League Cup Winner",
    "Kazakh Super Cup Winner",
    "Kosovan Champion",
    "Kosovan Super Cup Winner",
    "Kyrgyzstan Supercup Winner",
    "Kuwait Champion",
    "Kuwait Crown Prince Cup Winner",
    "Kuwait Emir Cup Winner",
    "Kuwait Federation Cup Winner",
    "Kuwait League Cup winner",
    "Kuwait Super Cup Winner",
    "Latvian champion",
    "Latvian cup winner",
    "Lebanese champion",
    "Lebanese cup winner",
    "Libyan Champion",
    "Liechtenstein Cup Winner",
    "Lithuanian champion",
    "Lithuanian cup winner",
    "Lithuanian Super Cup Winner",
    "Luxembourgish champion",
    "Luxembourgish cup winner",
    "Luxemburg League Cup Winner",
    "MLS Cup Champion",
    "MLS is Back Champion",
    "MTN8 Cup Winner",
    "Macedonian champion",
    "Malaysia Cup Winner",
    "Malaysian Champions",
    "Malaysian FA Cup Winner",
    "Malaysian Supercup Winner",
    "Maldivian Champion",
    "Maltese champion",
    "Maltese cup winner",
    "Maltese Super Cup winner",
    "Mexican Campeón de Campeones",
    "Mexican Champion Apertura",
    "Mexican Clausura champion",
    "Mexican Cup winner Apertura",
    "Mexican Cup winner Clausura",
    "Mexican Super Cup Winner",
    "Mexican champion",
    "Mexican cup winner",
    "Moldavian champion",
    "Moldavian cup winner",
    "Moldovan supercup winner",
    "Mongolian Champion",
    "Montenegrin champion",
    "Montenegrin cup winner",
    "Moroccan champion",
    "Moroccan cup winner",
    "Myanmarian Champion",
    "Myanmarian Cup Winner",
    "Myanmarian Supercup Winner",
    "NASL Regular Season Champion",
    "NASL Soccer Bowl Champion",
    "Nedbank Cup Winner",
    "Nepal Super League Winner",
    "New Zealand champion",
    "New Zealand Super Cup winner",
    "Nicaraguan Champion Apertura",
    "Nicaraguan Cup Winner",
    "Nigerian champion",
    "Northern Irish champion",
    "Northern Irish cup winner",
    "North Korean Champion",
    "North Korean Cup Winner",
    "Norwegian Super Cup Winner",
    "Norwegian champion",
    "Norwegian cup winner",
    "Omanian Champion",
    "Omanian Cup Winner",
    "Omanian League Cup Winner",
    "Omanian Supercup Winner",
    "Palestinian Cup Winner",
    "Palestinian West Bank Cup Winner",
    "Palestinian West Bank Premier League Champion",
    "Panamanian Champion",
    "Panamanian Champion Apertura",
    "Panamanian Champion Clausura",
    "Paraguayan Champion",
    "Paraguayan Champion Apertura",
    "Paraguayan Champion Clausura",
    "Peruvian Supercup winner",
    "Peruvian champion",
    "Peruvian cup winner",
    "Piala Presiden Winner",
    "Polish Super Cup winner",
    "Polish champion",
    "Polish cup winner",
    "Portuguese Super Cup winner",
    "Portuguese champion",
    "Portuguese cup winner",
    "Portuguese league cup winner",
    "Puerto Rican champion",
    "Qatari Cup Winner (Emir of Qatar Cup)",
    "Qatari League Cup Winner",
    "Qatari Stars Cup Winner",
    "Qatari Super Cup Winner (Sheikh Jassim Cup)",
    "Qatari champion",
    "Romanian League Cup Winner",
    "Romanian Super Cup winner",
    "Romanian champion",
    "Romanian cup winner",
    "Russian Super Cup winner",
    "Russian champion",
    "Russian cup winner",
    "Russian league cup winner",
    "Salvadoran Champion",
    "Salvadoran Champion Apertura",
    "Salvadoran Champion Clausura",
    "Salvadoran Supercup Winner",
    "San Marinese champion",
    "San Marinese cup winner",
    "San Marinese Supercup Winner",
    "Saudi Arabian champion",
    "Saudi Crown Prince Cup Winner",
    "Saudi Cup Winner",
    "Saudi Super Cup Winner",
    "Schweizer Ligapokalsieger",
    "Scottish champion",
    "Scottish cup winner",
    "Scottish league cup winner",
    "Senegalese cup winner",
    "Serbian champion",
    "Serbian cup winner",
    "Singaporean Super Cup winner",
    "Singaporean champion",
    "Singaporean cup winner",
    "Singaporean league cup winner",
    "Slovak champion",
    "Slovak cup winner",
    "Slovenian champion",
    "Slovenian cup winner",
    "South African champion",
    "South African cup winner",
    "South Korean Cup Winner",
    "South Korean League Cup Winner",
    "South Korean League Cup Winner (KNL)",
    "South Korean Super Cup Winner",
    "South Korean champion",
    "Soviet champion",
    "Soviet cup winner",
    "Spanish Super Cup winner",
    "Spanish champion",
    "Spanish cup winner",
    "Spanish league cup winner",
    "Supercopa Uruguaya Winner",
    "Supporters' Shield Winner",
    "Swedish Super Cup winner",
    "Swedish champion",
    "Swedish cup winner",
    "Swiss Super Cup Winner",
    "Swiss champion",
    "Swiss cup winner",
    "Syrian Champion",
    "Syrian Cup Winner",
    "Syrian Supercup Winner",
    "Taiwanese Champion",
    "Tajikistan Champion",
    "Tajikistan Super Cup Winner",
    "Tanzanian Supercup winner",
    "Thai Champion",
    "Thai Cup Winner",
    "Thai Kor Royal Cup winner",
    "Thai League Cup Winner",
    "Thailand Champions Cup Winner",
    "Tehran Clubs League champion",
    "Tunisian Champion",
    "Tunisian Super Cup Winner",
    "Tunisian cup winner",
    "Turkish Super Cup winner",
    "Turkish champion",
    "Turkish cup winner",
    "UAE Champion",
    "UAE Cup winner",
    "UAE League Cup Winner",
    "UAE President's Cup winner",
    "UAE Supercup Winner",
    "US Open Cup Winner",
    "Ukrainian Super Cup winner",
    "Ukrainian champion",
    "Ukrainian cup winner",
    "Uruguayan champion",
    "Uruguayan Cup Winner",
    "Uzbek Champion",
    "Uzbek Cup Winner",
    "Uzbek Supercup Winner",
    "Venezuelan Champion",
    "Venezuelan Cup Winner",
    "Vietnamese Champion",
    "Vietnamese Cup winner",
    "Vietnamese Super cup winner",
    "Welsh Champion",
    "Winner Copa Bicentenario",
    "Winner Copa Campeonato",
    "Winner Copa Centenario",
    "Winner Copa de la Liga Profesional",
    "Winner Copa del Inca",
    "Winner Copa LPF Proyección Inicial",
    "Winner Coupe Charles Drago",
    "Winner ISL Regular Season",
    "Winner Supercopa do Brasil",
    "Yugoslav cup winner",
    "Yugoslavian champion",
]

MINOR_TITLE = [
    "Albanian second tier champion",
    "Australian Regular Season Champion",
    "Austrian Central Regionalliga champion",
    "Austrian Eastern Regionalliga champion",
    "Austrian Second League Champion",
    "Austrian Western Regionalliga champion",
    "Austrian Youth league U16 champion",
    "Austrian Youth league U18 champion",
    "Austrian Youth league U19 champion",
    "Baden Cup winner",
    "Bavarian Cup winner",
    "Belgian Second League Champion",
    "Berlin Cup winner",
    "Bhutanese Champion",
    "Bosnian-Herzegovinian Champion U17",
    "Bosnian-Herzegovinian Champion U19",
    "Bosnian-Herzegovinian Cup Winner U19",
    "Brandenburg Cup winner",
    "Bremen Cup winner",
    "Bremenliga Champion",
    "Burgenland League champion",
    "CPL Shield Winner",
    "Campeonato Brasileiro Série B Champioen",
    "Campeão Carioca",
    "Campeão Gaúcho",
    "Campeão Mineiro",
    "Campeão Paulista",
    "Campeão da Copa do Nordeste",
    "Campeão da Taça Rio",
    "Campeón Primera Nacional",
    "Campeón Trofeo de campeones Proyección",
    "Campione Serie C2 (A)",
    "Campione Serie C2 (B)",
    "Campione Serie C2 (C)",
    "Challenger Cup Champion",
    "Champion 2nd League Portugal",
    "Champion Primera B",
    "Champion Torneo Federal A",
    "Champion Tweede Divisie",
    "China 2nd tier champion",
    "Chugoku Soccer League Champion",
    "Copa Catalunya",
    "Copa São Paulo de Juniores winner",
    "Coppa Italia Primavera winner",
    "Costa Rican Second Tier Champion",
    "Croatian Second League Champion",
    "Cypriot 2nd tier champion",
    "Czech 2nd Division Champion",
    "Czech U19 Champion",
    "Danish Youth Champion",
    "Danish reserve league winner",
    "Danish second tier Youth Champion",
    "Danish second tier champion",
    "Danish third tier champion",
    "Dutch amateur champion",
    "Dutch U17 Champion",
    "Dutch U19 Champion",
    "Dutch U19 Youth Cup winner",
    "Dutch second tier champion",
    "Ecuadorian 2nd tier champion",
    "English 2nd tier champion",
    "English 3rd tier champion",
    "English 4th tier champion",
    "English FA Youth Cup winner",
    "English Youth League winner (U18)",
    "Estonian second tier champion",
    "Estonian third tier champion",
    "First league Lower Austria champion",
    "Football League Trophy Winner",
    "French 2nd tier champion",
    "French Youth Cup winner",
    "French U19 champion",
    "German Regionalliga Bavaria Champion",
    "German Regionalliga North Champion",
    "German Regionalliga Northeast Champion",
    "German Regionalliga Southwest Champion",
    "German Regionalliga West Champion",
    "German Under-17 Bundesliga North/North-east champion",
    "German Under-17 Bundesliga South/South-west champion",
    "German Under-17 Bundesliga West champion",
    "German Under-17 Bundesliga champion",
    "German Under-19 Bundesliga North/North-east champion",
    "German Under-19 Bundesliga South/South-west champion",
    "German Under-19 Bundesliga West champion",
    "German Under-19 Bundesliga champion",
    "German Under-19 cup winner",
    "German amateur champion",
    "German second tier champion",
    "German third tier champion",
    "Greek second tier Super Cup winner",
    "Greek second tier champion",
    "Hamburg Cup winner",
    "Hessen Cup winner",
    "Hokkaido Soccer League Champion",
    "Hokushin'etsu Football League Champion",
    "Hong Kong Lower Division Cup Winner",
    "Hong Kong Second League Champion",
    "Indian Second Division Champion",
    "Indonesian Second Division Champion",
    "Indonesian 3rd tier champion",
    "Israeli 2nd tier champion",
    "Italian Supercoppa winner (Primavera)",
    "Italian Supercoppa winner (Under 17)",
    "Italian Youth champion (Primavera)",
    "Italian Youth champion (U17)",
    "Italian cup winner (Serie C)",
    "Italienischer Drittligameister (A)",
    "Italienischer Drittligameister (B)",
    "Italienischer Drittligameister (C)",
    "Italienischer Zweitligameister",
    "Japanese Amateur Cup Winner",
    "Japanese Prince Takamado U18 Champion",
    "Japanese Regional Champion",
    "Japanese second league Champion",
    "Japanese third league Champion",
    "K4 League Champion",
    "Kansai Soccer League Champion",
    "Kanto Soccer League Champion",
    "Kazakh 2nd tier champion",
    "Korean K League 2 Champion",
    "Korean National League Champion (KNL)",
    "Kyushu Soccer League Champion",
    "Kärntnen league champion",
    "Landespokal Niedersachsen (Amateure) Winner",
    "Leinster Senior Cup Champion",
    "Lower Rhine Cup winner",
    "Lower Saxony Champion",
    "Lower Saxony Cup winner",
    "Meister Oberliga Südwest",
    "Mecklenburg-Western Pomerania Cup winner",
    "Middle Rhine Cup winner",
    "Minor Premiership winner (NZL)",
    "MLS Next Pro Cup Champion",
    "Munster Senior Cup Champion",
    "Northern German Cup Winner",
    "Peruvian second tier champion",
    "Polish U19-Champion",
    "Portugese Under 23-Champion",
    "Portuguese Third Division Champion",
    "Premier League Cup-Winner",
    "Premier League International Cup winner",
    "Qatari Second Division Champion",
    "Regions Cup Winner",
    "Rhineland Cup winner",
    "Romanian 2nd Division Champion",
    "Russian U16 Champion",
    "Russian U17 Champion",
    "Russian U19 Champion",
    "Russian second tier champion",
    "Russian third tier champion",
    "Russian fourth tier champion",
    "Saarland Cup winner",
    "Salzburg league champion",
    "Saudi Arabian 2nd tier champion",
    "Saudi Arabian 3rd tier champion",
    "Saxony Cup winner",
    "Saxony-Anhalt Cup winner",
    "Schleswig-Holstein Cup winner",
    "Scottish Challenge Cup winner",
    "Scottish Fourth League Champion",
    "Scottish Second League Champion",
    "Scottish Third League Champion",
    "Scudetto Serie D",
    "Serbia U17 Champion",
    "Serbia U19 Champion",
    "Serbian Second League Champion",
    "Shikoku Soccer League Champion",
    "Sieger Campeonato de Portugal",
    "Slovak second tier champion",
    "South Korean 3rd tier champion",
    "Soviet Second Division Champion",
    "Spanish 2nd tier champion",
    "Steiermark league champion",
    "Sudbaden Cup winner",
    "Sudwest Cup winner",
    "Supercoppa Serie C winner",
    "Supercoppa Serie C2 Winner",
    "Swedish Superettan champion",
    "Swiss 2nd tier champion",
    "Swiss 3rd tier champion",
    "Swiss U18-Champion",
    "Thai Second League Champion",
    "Thai Third League Champion",
    "Thuringia Cup winner",
    "Tohoku Soccer League Champion",
    "Tokai Soccer League Champion",
    "Torneio Internacional Algarve U17",
    "Turkish 3rd division champion",
    "Turkish 4th division champion",
    "Turkish second tier champion",
    "Türkischer U19 Meister",
    "Türkischer U21 Meister",
    "U20 Copa Libertadores winner",
    "U20 Elite League Winner",
    "U20 Intercontinental Cup Winner",
    "U21 Eredivisie champion",
    "U21 Premier League champion",
    "UEFA Youth League Winner",
    "Ukrainian U19 champion",
    "Ukrainian second tier champion",
    "Ukrainian third tier champion",
    "Under-17 Brazilian champion",
    "Under-20 Brazilian champion",
    "Upper Austria league champion",
    "Uruguayan 2nd tier Champion",
    "USL Championship Champion",
    "USL Regular Season Champion",
    "USL-1 Cup Champion",
    "USL1 Cup Champion",
    "USL-1 Regular Season Champion",
    "USL1 Regular Season Champion",
    "Uzbek 2nd tier Champion",
    "Viareggio",
    "Vienna City League champion",
    "Vietnam 2nd tier champion",
    "Vincitore Coppa Italia (Serie D)",
    "Vorarlberg league champion",
    "Western German Cup Winner",
    "Westphalia Cup winner",
    "Winner CONCACAF U17-Championship",
    "Winner Copa LPF Proyección Final",
    "Winner Copa RFEF",
    "Winner Italian Supercoppa (Primavera2)",
    "Winner Japanese Prince Takamado U18 Premier League East",
    "Winner of the Japanese Amateur championship",
    "Winner Piala Kemenpora",
    "Wurttemberg Cup winner",
]
